...
TESTING_FILE_PATH = r"D:\New folder (2)\working python projects\outlook\process\profile_automation.py"
...
def testing_gui(category, process):
    def execute_testing_process():
        status_label.config(text="Processing Testing Task...", fg="orange")
        try:
            # Use exec to run the profile automation script
            with open(TESTING_FILE_PATH, "r") as file:
                exec(file.read())
            status_label.config(text="Testing Process Completed", fg="green")
        except Exception as e:
            status_label.config(text=f"Testing Process Failed: {e}", fg="red")
    ...
    root = tk.Tk()
    root.geometry(WINDOW_SIZE)
    root.title("Testing Process Execution")
    root.configure(bg=BG_COLOR)

    tk.Label(root, text=f"Testing File Path: {TESTING_FILE_PATH}", bg=BG_COLOR, fg=FG_COLOR, font=("Arial", 12, "bold")).pack(pady=5)
    tk.Label(root, text=f"Category: {category} | Process: {process}", bg=BG_COLOR, fg=FG_COLOR, font=("Arial", 12, "bold")).pack(pady=10)

    run_btn = tk.Button(root, text="Run", command=execute_testing_process, bg=BUTTON_COLOR, fg=FG_COLOR, font=("Arial", 12, "bold"), width=15)
    run_btn.pack(pady=20)

    exit_btn = tk.Button(root, text="Exit", command=exit_to_process_gui, bg=BUTTON_COLOR, fg=FG_COLOR, font=("Arial", 12, "bold"), width=15)
    exit_btn.pack(pady=10)

    status_label = tk.Label(root, text="Awaiting Testing Input", bg=BG_COLOR, fg=FG_COLOR, font=("Arial", 12, "bold"))
    status_label.pack(pady=5)
